...
<html>
<head>
	<title>Пассивный заработок денег в интернете, способы получения пассивного дохода</title>
	<meta name="description" content="Мы покажем способы пассивного заработка денег в интернете, которые позволят вам получать доход без вложений своего времени и сил">
	<meta charset = "utf-8">
...
<form method="POST" action=<?php echo $_SERVER['PHP_SELF'];?> enctype="multipart/form-data">
	...
	<section class="container">
		<div class="text katalog-text">
			<h1 class="text__title">Пассивный заработок в интернете</h1>
			<p class="text__descript">
				Пассивный заработок - это такой виды получение дохода который устроит почти каждого пользователя интернета.
				Такой пассивный заработок денег доступен многим пользователям интернета.
				Категория такого получения дохода будет полезна многим желающим заработать деньги.
				На просторах интернета заработать вполне реально и существует множество способов, главное иметь терпение и время,
				что бы тестировать различные методы пассивного заработка. У нас на сайте представлены различные методы пассивного заработка,
				где можно зарабатывать без вложений, а где-то придется раскошелиться.
			</p>
		</div>
		<center>
		...
		<div class="content">
			...
			<?php
				...
				$state = mysqli_query($conect,"SELECT `name`, `image`, `title-katalog`, `description-katalog`, `money-level`, `hard-level`, `type` FROM `post` WHERE `passiv` = 'on'");
